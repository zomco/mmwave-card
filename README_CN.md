...
已注册 17 个适配器。只测距的型号只有距离没有方向，所以渲染成一段圆弧而不是一个点，
也无法参与多雷达融合。
...
可选的 AI 默认关闭，要在 Home Assistant 里打开才会用。在对话代理中勾选 **MMWave Fusion**
LLM API，即可用 Assist 问占用和事件（轨迹始终匿名）。若在集成选项里填了 AI Task 实体，
穿越短片会带上 有人/宠物/误报 标签并显示在事件列表。偏航校准页和融合警告会写明常见的
90°/180° 错误，这一步不需要模型。
