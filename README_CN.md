...
多雷达融合还需要另外安装 **[mmwave-fusion](https://github.com/zomco/mmwave-fusion)** 集成（HACS 中是独立的一项，类别为 integration）。装了它才有：轨迹持久化、区域事件、摄像头录像。

未安装时，卡片仍会渲染融合视图，但融合只发生在浏览器内、不保存任何数据 —— 此时卡片会明确提示"未安装融合集成"，而不是假装一切正常。

两者独立发版，因此集成会在每次推送里带上 `api_version`，卡片发现后端版本低于所需时会直接提示升级。
