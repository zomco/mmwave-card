...
`,Oe="mmwave-card",Be="mmwave-card-editor",Ue=15e3,Ke=new WeakMap;function Ye(t,e,i,a){if(!e||!1===e.visible)return{point:t};const r=fe(e.url);if("ready"!==r?.status)return{point:t};const o=r.image;if(!Ke.has(o))try{const t=Math.min(1,1024/Math.max(o.naturalWidth,o.naturalHeight)),e=document.createElement("canvas");e.width=Math.max(1,Math.round(o.naturalWidth*t)),e.height=Math.max(1,Math.round(o.naturalHeight*t));const i=e.getContext("2d",{willReadFrequently:!0});i.drawImage(o,0,0,e.width,e.height),Ke.set(o,function(t,e,i){const a=new Float32Array(e*i),r=new Uint8Array(e*i);for(let e=0;e<a.length;e++){const i=t[4*e+3]/255;a[e]=(.299*t[4*e]+.587*t[4*e+1]+.114*t[4*e+2])*i+255*(1-i)}for(let t=1;t<i-1;t++)for(let i=1;i<e-1;i++){const o=t*e+i;r[o]=Math.max(Math.abs(a[o+1]-a[o-1]),Math.abs(a[o+e]-a[o-e]))>=60?1:0}return{width:e,height:i,edges:r}}(i.getImageData(0,0,e.width,e.height).data,e.width,e.height))}catch{Ke.set(o,null)}const s=Ke.get(o);if(!s)return{point:t,unavailable:!0};const n=be(t,e,i),l=function(t,e,i){let a,r=i*i;for(let o=Math.max(0,Math.floor(e.y-i));o<=Math.min(t.height-1,e.y+i);o++)for(let s=Math.max(0,Math.floor(e.x-i));s<=Math.min(t.width-1,e.x+i);s++){const i=(s-e.x)**2+(o-e.y)**2;t.edges[o*t.width+s]&&i<r&&(r=i,a={x:s,y:o})}return a}(s,{x:n.x*s.width,y:n.y*s.width},Math.min(40,a/me(e,i).width*s.width));return l?{point:ye({x:l.x/s.width,y:l.y/s.width},e,i),snapped:!0}:{point:t}}const Xe={position:"#03a9f4",height:"#00a878",yaw:"#ff9800",pitch:"#7e57c2",roll:"#ec407a"},Ze=(t,e,i)=>Math.min(i,Math.max(e,t)),Ge=t=>t*Math.PI/180,Ve=(t,e)=>Math.round(t/e)*e;let Je=class extends dt{constructor(){super(...arguments),this.lang="en",this.roomW=400,this.roomD=350,this.peerCalibrations=[],this._handles=new Map,this._drawRaf=0,this.floorplanLoaded=()=>{this.isConnected&&this._scheduleDraw()}}_t(t,e){return Ie(t,this.lang,e)}get _verticalFovDegrees(){return this.adapter?.info.verticalFovDegrees??Math.min(this.adapter?.info.fovDegrees??60,60)}get _isVerticalFovEstimated(){return null==this.adapter?.info.verticalFovDegrees}firstUpdated(){this._cv&&(this._resizeObserver=new ResizeObserver(()=>this._scheduleDraw()),this._resizeObserver.observe(this._cv)),this._scheduleDraw()}updated(){this._scheduleDraw()}disconnectedCallback(){super.disconnectedCallback(),this._resizeObserver?.disconnect(),cancelAnimationFrame(this._drawRaf)}_scheduleDraw(){cancelAnimationFrame(this._drawRaf),this._drawRaf=requestAnimationFrame(()=>this._draw())}_scene(){const t=this._cv?.offsetWidth||420,e=Ze(Math.round(.7*t),260,330);return{W:t,H:e,floorW:Math.max(180,t-72),floorH:Math.min(104,.32*e),floorTop:.48*e,verticalH:.36*e,roomW:this.calibration?.room_w??this.roomW,roomD:this.calibration?.room_d??this.roomD,zMax:400}}_project(t,e){const i=t.x/e.roomW,a=t.y/e.roomD,r=t.z/e.zMax;return{x:e.W/2+(i-a)*(e.floorW/2),y:e.floorTop+(i+a)*(e.floorH/2)-r*e.verticalH}}_unproject(t,e,i){const a=(t.x-i.W/2)/(i.floorW/2),r=(t.y+e/i.zMax*i.verticalH-i.floorTop)/(i.floorH/2);return{x:(a+r)/2*i.roomW,y:(r-a)/2*i.roomD}}_polygon(t,e){t.beginPath(),e.forEach((e,i)=>0===i?t.moveTo(e.x,e.y):t.lineTo(e.x,e.y)),t.closePath()}_line(t,e,i){t.beginPath(),t.moveTo(e.x,e.y),t.lineTo(i.x,i.y),t.stroke()}_drawHandle(t,e,i,a){this._handles.set(e,i),t.save(),t.shadowColor=Xe[e],t.shadowBlur=this._drag?.mode===e?14:7,t.beginPath(),t.arc(i.x,i.y,this._drag?.mode===e?9:7,0,2*Math.PI),t.fillStyle=Xe[e],t.fill(),t.shadowBlur=0,t.strokeStyle="#fff",t.lineWidth=1.5,t.stroke(),t.font="bold 9px system-ui",t.textAlign="center",t.textBaseline="bottom",t.fillStyle=Xe[e],t.fillText(a,i.x,i.y-11),t.restore()}_draw(){const t=this._cv;if(!t||!this.calibration||0===t.offsetWidth)return;const e=this._scene(),i=Ee(t,e.H),a=getComputedStyle(this),r=a.getPropertyValue("--primary-text-color").trim()||"#374151",o=a.getPropertyValue("--secondary-text-color").trim()||"#6b7280",s=this.calibration,n=[this._project({x:0,y:0,z:0},e),this._project({x:e.roomW,y:0,z:0},e),this._project({x:e.roomW,y:e.roomD,z:0},e),this._project({x:0,y:e.roomD,z:0},e)],l=[this._project({x:0,y:0,z:e.zMax},e),this._project({x:e.roomW,y:0,z:e.zMax},e)];i.clearRect(0,0,e.W,e.H),this._handles.clear(),i.save(),this._polygon(i,[n[0],n[1],l[1],l[0]]),i.fillStyle="rgba(3,169,244,.035)",i.fill(),this._polygon(i,[n[0],n[3],this._project({x:0,y:e.roomD,z:e.zMax},e),l[0]]),i.fillStyle="rgba(11,130,92,.035)",i.fill(),i.restore(),this._polygon(i,n),i.fillStyle="rgba(11,130,92,.09)",i.fill(),i.save(),this._polygon(i,n),i.clip(),i.transform(e.floorW/(2*e.roomW),e.floorH/(2*e.roomW),-e.floorW/(2*e.roomD),e.floorH/(2*e.roomD),e.W/2,e.floorTop),xe(i,{W:e.roomW,H:e.roomD,roomW:e.roomW,roomD:e.roomD},this.floorplan,this.floorplanLoaded),i.restore(),this._polygon(i,n),i.strokeStyle="rgba(11,130,92,.55)",i.lineWidth=1.4,i.stroke(),i.save(),i.strokeStyle=o,i.globalAlpha=.14,i.lineWidth=.8;for(let t=.25;t<1;t+=.25)this._line(i,this._project({x:e.roomW*t,y:0,z:0},e),this._project({x:e.roomW*t,y:e.roomD,z:0},e)),this._line(i,this._project({x:0,y:e.roomD*t,z:0},e),this._project({x:e.roomW,y:e.roomD*t,z:0},e));i.restore(),i.save(),i.strokeStyle=o,i.globalAlpha=.25,i.setLineDash([3,4]);for(const t of[{x:0,y:0},{x:e.roomW,y:0},{x:0,y:e.roomD}])this._line(i,this._project({...t,z:0},e),this._project({...t,z:e.zMax},e));i.restore(),i.font="bold 10px system-ui",i.fillStyle=o,i.fillText("X",n[1].x+8,n[1].y+2),i.fillText("Y",n[3].x-14,n[3].y+2),i.fillText("Z",l[0].x-13,l[0].y-2);for(const t of this.peerCalibrations){const a=this._project({x:t.calibration.radar_x,y:t.calibration.radar_y,z:t.calibration.radar_z},e),r=Ge(t.calibration.yaw),s=this._project({x:t.calibration.radar_x+45*Math.sin(r),y:t.calibration.radar_y+45*Math.cos(r),z:t.calibration.radar_z},e);i.save(),i.globalAlpha=.48,i.strokeStyle=o,i.fillStyle=o,i.lineWidth=1.2,this._line(i,a,s),i.beginPath(),i.arc(a.x,a.y,4,0,2*Math.PI),i.fill(),i.font="bold 9px system-ui",i.textAlign="center",i.fillText(t.id,a.x,a.y-9),i.restore()}const d=this._project({x:s.radar_x,y:s.radar_y,z:0},e),c=this._project({x:s.radar_x,y:s.radar_y,z:s.radar_z},e),p=Ge(s.yaw),h=Ge(s.pitch),_=Ge(s.roll),u=Math.sin(p)*Math.cos(h),g=Math.cos(p)*Math.cos(h),m=-Math.sin(h),f=Math.cos(p),y=-Math.sin(p),b=0,v=-Math.sin(p)*Math.sin(h),x=-Math.cos(p)*Math.sin(h),w=-Math.cos(h),$=f*Math.cos(_)+v*Math.sin(_),k=y*Math.cos(_)+x*Math.sin(_),M=b*Math.cos(_)+w*Math.sin(_),S=v*Math.cos(_)-f*Math.sin(_),R=x*Math.cos(_)-y*Math.sin(_),z=w*Math.cos(_)-b*Math.sin(_);i.save(),i.strokeStyle=Xe.height,i.globalAlpha=.55,i.setLineDash([4,4]),i.lineWidth=1.5,this._line(i,d,c),i.restore(),i.save(),i.translate(d.x,d.y),i.scale(1,.42),i.beginPath(),i.arc(0,0,12,0,2*Math.PI),i.fillStyle="rgba(3,169,244,.14)",i.fill(),i.restore();const C=this.maxRangeM??this.adapter?.info.maxRangeM??3,D=Math.min(100*C,.58*Math.max(e.roomW,e.roomD)),A=Ge((this.adapter?.info.fovDegrees??60)/2),E=Ge(this._verticalFovDegrees/2),P=(t,i,a=D)=>{const r=Math.cos(i),o=u*Math.cos(t)*r+$*Math.sin(t)*r+S*Math.sin(i),n=g*Math.cos(t)*r+k*Math.sin(t)*r+R*Math.sin(i),l=m*Math.cos(t)*r+M*Math.sin(t)*r+z*Math.sin(i);return this._project({x:s.radar_x+o*a,y:s.radar_y+n*a,z:Ze(s.radar_z+l*a,0,e.zMax)},e)},W=t=>Array.from({length:19},(e,i)=>P(i/18*A*2-A,t)),T=t=>Array.from({length:9},(e,i)=>P(t,i/8*E*2-E)),F=W(-E),H=W(0),q=W(E),N=T(-A),L=T(A);i.save(),this._polygon(i,[c,...F]),i.fillStyle="rgba(3,169,244,.055)",i.fill(),this._polygon(i,[c,...q]),i.fillStyle="rgba(11,130,92,.055)",i.fill(),this._polygon(i,[c,...N]),i.fillStyle="rgba(3,169,244,.04)",i.fill(),this._polygon(i,[c,...L]),i.fill(),i.strokeStyle="rgba(3,169,244,.25)",i.lineWidth=.8;for(let t=0;t<=18;t+=3)this._line(i,F[t],q[t]);for(const t of[F,q,N,L])i.beginPath(),t.forEach((t,e)=>0===e?i.moveTo(t.x,t.y):i.lineTo(t.x,t.y)),i.stroke();this._polygon(i,[c,...H]),i.fillStyle="rgba(11,130,92,.16)",i.fill(),i.strokeStyle="rgba(11,130,92,.72)",i.lineWidth=1.25,i.stroke();const I=this.adapter?.info.minRangeM??0;if(I>0&&C>0){const t=D*Math.min(I/C,.8),e=Array.from({length:19},(e,i)=>P(i/18*A*2-A,0,t));i.beginPath(),e.forEach((t,e)=>0===e?i.moveTo(t.x,t.y):i.lineTo(t.x,t.y)),i.setLineDash([3,3]),i.strokeStyle="rgba(11,130,92,.48)",i.stroke(),i.setLineDash([])}i.restore();const j=(t,i)=>this._project({x:s.radar_x+$*t+S*i,y:s.radar_y+k*t+R*i,z:s.radar_z+M*t+z*i},e),O=[j(-22,-10),j(22,-10),j(22,10),j(-22,10)];this._polygon(i,O),i.fillStyle="#13212b",i.fill(),i.strokeStyle="#6ee7c1",i.lineWidth=1.5,i.stroke(),i.beginPath(),i.arc(c.x,c.y,4,0,2*Math.PI),i.fillStyle="#0b825c",i.fill();const B=.18*Math.min(e.roomW,e.roomD),U=t=>({x:Ze(t.x,18,e.W-18),y:Ze(t.y,52,e.H-18)}),K=U(this._project({x:s.radar_x+Math.sin(p)*B,y:s.radar_y+Math.cos(p)*B,z:s.radar_z},e));i.strokeStyle=Xe.yaw,i.lineWidth=2,this._line(i,c,K);const Y=U({x:c.x-28,y:c.y});i.strokeStyle=Xe.height,i.lineWidth=1,this._line(i,{x:Y.x+8,y:Y.y},c);const X=U({x:K.x,y:K.y-30-s.pitch/90*20});i.strokeStyle=Xe.pitch,i.setLineDash([2,3]),this._line(i,K,X),i.setLineDash([]);const Z=j(38,0),G=U({x:Z.x+s.roll/90*10,y:Z.y});this._drawHandle(i,"position",d,"XY"),this._drawHandle(i,"height",Y,"Z"),this._drawHandle(i,"yaw",K,this._t("install3d.yaw")),this._drawHandle(i,"pitch",X,this._t("install3d.pitch")),this._drawHandle(i,"roll",G,this._t("install3d.roll")),i.save(),i.fillStyle=r,i.globalAlpha=.72,i.font="10px system-ui",i.textAlign="right",i.fillText(`${Math.round(e.roomW)} × ${Math.round(e.roomD)} cm`,e.W-10,e.H-10),i.restore()}_hitTest(t){let e;for(const[i,a]of this._handles){const r=Math.hypot(t.x-a.x,t.y-a.y);r<=18&&(!e||r<e.distance)&&(e={mode:i,distance:r})}return e?.mode}_onPointerDown(t){const e=this._cv;if(!e)return;const i=Ae(t,e),a=this._hitTest(i);if(!a)return;t.preventDefault(),e.setPointerCapture(t.pointerId);const r="height"===a?this.calibration.radar_z:"yaw"===a?this.calibration.yaw:"pitch"===a?this.calibration.pitch:"roll"===a?this.calibration.roll:0;this._drag={mode:a,startX:i.x,startY:i.y,startValue:r},this._scheduleDraw()}_onPointerMove(t){const e=this._cv;if(!e)return;const i=Ae(t,e);if(!this._drag)return void(e.style.cursor=this._hitTest(i)?"grab":"default");t.preventDefault();const a=this._scene(),r=this._drag;if("position"===r.mode){const t=this._unproject(i,0,a);this._emit({radar_x:Ve(Ze(t.x,0,a.roomW),1),radar_y:Ve(Ze(t.y,0,a.roomD),1)})}else if("height"===r.mode){const t=r.startValue-(i.y-r.startY)/a.verticalH*a.zMax;this._emit({radar_z:Ve(Ze(t,0,a.zMax),1)})}else if("yaw"===r.mode){const t=this._unproject(i,this.calibration.radar_z,a),e=180*Math.atan2(t.x-this.calibration.radar_x,t.y-this.calibration.radar_y)/Math.PI;this._emit({yaw:Ve(e,.5)})}else if("pitch"===r.mode){const t=r.startValue-.6*(i.y-r.startY);this._emit({pitch:Ve(Ze(t,-90,90),.5)})}else{const t=r.startValue+.6*(i.x-r.startX);this._emit({roll:Ve(Ze(t,-90,90),.5)})}}_onPointerUp(t){const e=this._cv;e?.hasPointerCapture(t.pointerId)&&e.releasePointerCapture(t.pointerId),this._drag=void 0,this._scheduleDraw()}_emit(t){this.dispatchEvent(new CustomEvent("calibration-changed",{detail:{...this.calibration,...t},bubbles:!0,composed:!0}))}render(){if(!this.calibration)return B``;const t=this.calibration;return B`
  ...
  `}};t([ut({attribute:!1})],_i.prototype,"hass",void 0),t([ut({attribute:!1})],_i.prototype,"config",void 0),t([ut({attribute:!1})],_i.prototype,"floorplan",void 0),t([ut({attribute:!1})],_i.prototype,"adapter",void 0),t([ut({attribute:!1})],_i.prototype,"calibration",void 0),t([ut({attribute:!1})],_i.prototype,"lang",void 0),t([ut({type:Number})],_i.prototype,"roomW",void 0),t([ut({type:Number})],_i.prototype,"roomD",void 0),t([ut({attribute:!1})],_i.prototype,"targets",void 0),t([ut({type:Boolean})],_i.prototype,"present",void 0),t([ut({type:Boolean})],_i.prototype,"showStatus",void 0),t([ut({type:Number})],_i.prototype,"maxRangeM",void 0),t([ut({attribute:!1})],_i.prototype,"areas",void 0),t([ut({attribute:!1})],_i.prototype,"areaOccupied",void 0),t([ut({type:Boolean})],_i.prototype,"privacy",void 0),t([mt("#live-cv")],_i.prototype,"_cv",void 0),_i=t([pt("mmwave-live-panel")],_i);let ui=class extends dt{constructor(){super(...arguments),this.roomW=400,this.roomD=600,this.areas=[{polygon:[]},{polygon:[]},{polygon:[]}],this.lang="en",this.selected=0,this.floorplanLoaded=()=>{this.isConnected&&this.requestUpdate()}}t(t,e){return Ie(t,this.lang,e)}connectedCallback(){super.connectedCallback(),this.floorplan?.url&&fe(this.floorplan.url,this.floorplanLoaded)}emit(t){this.dispatchEvent(new CustomEvent("areas-changed",{detail:t,bubbles:!0,composed:!0}))}patchSelected(t){const e=this.areas.map((e,i)=>i===this.selected?{polygon:t}:e);this.emit(e)}addPoint(t){const e=t.currentTarget.getBoundingClientRect(),i={x:Math.round(Math.min(Math.max((t.clientX-e.left)/e.width*this.roomW,0),this.roomW)),y:Math.round(Math.min(Math.max((t.clientY-e.top)/e.height*this.roomD,0),this.roomD))};this.patchSelected([...this.areas[this.selected]?.polygon??[],i])}undo(){const t=this.areas[this.selected]?.polygon??[];t.length&&this.patchSelected(t.slice(0,-1))}clear(){this.patchSelected([])}standHere(){this.standPoint&&this.patchSelected(function(t,e,i,a=180){const r=a/2;let o=t.x-r,s=t.y-r,n=t.x+r,l=t.y+r;return o<0&&(n-=o,o=0),s<0&&(l-=s,s=0),n>e&&(o-=n-e,n=e),l>i&&(s-=l-i,l=i),o=Math.max(0,Math.round(o)),s=Math.max(0,Math.round(s)),n=Math.min(e,Math.round(n)),l=Math.min(i,Math.round(l)),[{x:o,y:s},{x:n,y:s},{x:n,y:l},{x:o,y:l}]}(this.standPoint,this.roomW,this.roomD))}pointString(t){return t.map(t=>`${t.x},${t.y}`).join(" ")}render(){const t=!1!==this.floorplan?.visible&&this.floorplan?fe(this.floorplan.url,this.floorplanLoaded):void 0,e=me(this.floorplan??{},this.roomW),i=function(t){const e=t.map((t,e)=>({polygon:t,index:e,center:$e(t)})).filter(t=>t.center&&t.polygon.length>=3),i=[];for(const t of e)ke(t.polygon)<150&&i.push(`small:${t.index}`);for(let t=0;t<e.length;t++)for(let a=t+1;a<e.length;a++){const r=e[t].center.x-e[a].center.x,o=e[t].center.y-e[a].center.y;Math.hypot(r,o)<200&&i.push(`close:${e[t].index}:${e[a].index}`),Me(e[t].polygon,e[a].polygon)&&i.push(`overlap:${e[t].index}:${e[a].index}`)}return i}(this.areas.map(t=>t.polygon));return B`
      ...
      >
        ...
        ${this.areas.map((t,e)=>{const i=we[e],a=this.selected===e;return t.polygon.length?U`
            ${t.polygon.length>=3?U`<polygon points=${this.pointString(t.polygon)} fill=${i} fill-opacity=${a?".22":".08"} stroke=${i} stroke-width=${a?3:2} vector-effect="non-scaling-stroke" />`:U`<polyline points=${this.pointString(t.polygon)} fill="none" stroke=${i} stroke-width="3" vector-effect="non-scaling-stroke" />`}
            ${t.polygon.map(t=>U`<circle cx=${t.x} cy=${t.y} r="7" fill=${i} stroke="white" stroke-width="2" vector-effect="non-scaling-stroke" />`)}
          `:Y})}
        ${this.standPoint?U`<circle cx=${this.standPoint.x} cy=${this.standPoint.y} r="14" fill="none" stroke="white" stroke-width="2" stroke-dasharray="4 3" vector-effect="non-scaling-stroke" />`:Y}
        ${this.radar?U`<g transform=${`translate(${this.radar.x} ${this.radar.y}) rotate(${-this.radar.yaw})`}><circle r="10" fill="#111" /><path d="M 0 0 L -10 22 M 0 0 L 10 22" stroke="#111" fill="none" /></g>`:Y}
      </svg>
      <div class="actions">
        ...
        <div class="rules-grid">
          ...
          <div class="field compact">
            ...
            <input
              ...
              min="0"
              step="5"
              .value=${String(this._config.fusion?.duplicate_gate_cm??Ci("duplicate_gate_cm",e))}
          ...
          <div class="field compact">
            ...
            <input
              ...
              min="0"
              step="0.01"
              .value=${String(this._config.fusion?.range_merge_factor??Ci("range_merge_factor",e))}
          ...
          <div class="field compact">
            ...
            <input
              ...
              min="0"
              step="0.1"
              .value=${String(this._config.fusion?.merge_confirm_s??Ci("merge_confirm_s",e))}
