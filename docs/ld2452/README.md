# ld2452 Lovelace Configuration Guide

[中文文档 (Chinese)](./README_CN.md)

This guide explains how to use the configuration file located in `tests/ld2452.yaml` within Home Assistant.

## 1. Introduction

`ld2452.yaml` contains the basic Lovelace card configuration for the Hi-Link ld2452 mmWave multi-target tracking radar using [mmwave-card](https://github.com/zomco/mmwave-card). It enables visual real-time tracking of up to 3 targets and geometric boundary calibration display for this radar in Home Assistant.
...
2. Your ld2452 radar device is successfully integrated into Home Assistant via ESPHome or other methods.
...
5. Copy the contents from `tests/ld2452.yaml` and paste them into the code editor.
...
All `entity_id`s in the configuration (such as `binary_sensor.ld2452_presence`) are examples. If your device has different names in Home Assistant, please make sure to replace them with **your actual entity IDs**.
...
- **`radar_model`**: `ld2452` (Fixed value, specifies the radar model used)
