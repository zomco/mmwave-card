# LD2450A Lovelace Configuration Guide

[中文文档 (Chinese)](./README_CN.md)

This guide explains how to use the configuration file located in `tests/ld2450a.yaml` within Home Assistant.

## 1. Introduction

`ld2450a.yaml` contains the basic Lovelace card configuration for the Hi-Link LD2450A 24 GHz presence and gesture recognition radar using [mmwave-card](https://github.com/zomco/mmwave-card). It enables visual real-time distance tracking, gesture recognition monitoring, and geometric boundary calibration display for this radar in Home Assistant.
...
2. Your LD2450A radar device is successfully integrated into Home Assistant via ESPHome or other methods.

## 3. Hardware Wiring

LD2450A connects to the MCU via TTL UART (default baud rate 256000, 8N1):

| LD2450A Pin | ESP32-C3 Pin | Description |
| :--- | :--- | :--- |
| 5V | 5V | Power Supply (requires > 200mA capability) |
| GND | GND | Ground |
| TX | GPIO20 (RX) | UART Transmit from Radar |
| RX | GPIO21 (TX) | UART Receive to Radar |
...
5. Copy the contents from `tests/ld2450a.yaml` and paste them into the code editor.
...
All `entity_id`s in the configuration (such as `binary_sensor.ld2450a_presence`) are examples. If your device has different names in Home Assistant, please make sure to replace them with **your actual entity IDs**.
...
- **`radar_model`**: `ld2450a` (Fixed value, specifies the radar model used)
- **`presence_entity`**: [Required] Binary sensor indicating whether any human target is present.
- **`distance_entity`**: [Required] Radar-measured line-of-sight distance sensor (Unit: cm). Because the LD2450A is a 1-D ranging radar for presence, the card automatically maps this distance straight ahead. You can use the card's `Yaw` slider to adjust the radar's physical orientation in the room.
