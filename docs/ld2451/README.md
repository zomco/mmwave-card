# LD2451 Lovelace Configuration Guide

[中文文档 (Chinese)](./README_CN.md)

This guide explains how to use the configuration file located in `tests/ld2451.yaml` within Home Assistant.

## 1. Introduction

`ld2451.yaml` contains the basic Lovelace card configuration for the Hi-Link LD2451 24 GHz 2D multi-target tracking radar using [mmwave-card](https://github.com/zomco/mmwave-card). It enables visual real-time tracking of up to 3 targets and geometric boundary calibration display for this radar in Home Assistant.
...
2. Your LD2451 radar device is successfully integrated into Home Assistant via ESPHome or other methods.
...
5. Copy the contents from `tests/ld2451.yaml` and paste them into the code editor.
...
cards:
  - type: custom:mmwave-card
    radar_model: ld2451
    presence_entity: binary_sensor.ld2451_presence
    target_1_x_entity: sensor.ld2451_target_1_x
    target_1_y_entity: sensor.ld2451_target_1_y
    target_1_speed_entity: sensor.ld2451_target_1_speed
    target_2_x_entity: sensor.ld2451_target_2_x
    target_2_y_entity: sensor.ld2451_target_2_y
    target_2_speed_entity: sensor.ld2451_target_2_speed
    target_3_x_entity: sensor.ld2451_target_3_x
    target_3_y_entity: sensor.ld2451_target_3_y
    target_3_speed_entity: sensor.ld2451_target_3_speed
    room_w: 400
    room_d: 350
```
...
All `entity_id`s in the configuration (such as `binary_sensor.ld2451_presence`) are examples. If your device has different names in Home Assistant, please make sure to replace them with **your actual entity IDs**.
...
- **`radar_model`**: `ld2451` (Fixed value, specifies the radar model used)
