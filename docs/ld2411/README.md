# LD2411 Lovelace Configuration Guide

[中文文档 (Chinese)](./README_CN.md)

This guide explains how to use the configuration file located in `tests/ld2411.yaml` within Home Assistant.

## 1. Introduction

`ld2411.yaml` contains the basic Lovelace card configuration for the Hi-Link LD2411 24 GHz 1-D ranging radar using [mmwave-card](https://github.com/zomco/mmwave-card). It enables visual real-time distance tracking, motion state monitoring, and geometric boundary calibration display for this radar in Home Assistant.
...
2. Your LD2411 radar device is successfully integrated into Home Assistant via ESPHome or other methods.
...
5. Copy the contents from `tests/ld2411.yaml` and paste them into the code editor.
...
cards:
  - type: custom:mmwave-card
    radar_model: ld2411
    presence_entity: binary_sensor.ld2411_presence
    distance_entity: sensor.ld2411_distance
    motion_state_entity: sensor.ld2411_motion_state
    room_w: 400
    room_d: 350
```
...
All `entity_id`s in the configuration (such as `binary_sensor.ld2411_presence`) are examples. If your device has different names in Home Assistant, please make sure to replace them with **your actual entity IDs**.
...
- **`radar_model`**: `ld2411` (Fixed value, specifies the radar model used)
- **`presence_entity`**: [Required] Binary sensor indicating whether any target is present.
- **`distance_entity`**: [Required] Radar-measured line-of-sight distance sensor (Unit: cm). Because the LD2411 is a 1-D radar, the card automatically maps this distance straight ahead. You can use the card's `Yaw` slider to adjust the radar's physical orientation in the room.
