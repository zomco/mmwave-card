...
占位区域（书桌/床/门口）在校准的「实时」页绘制，保存到 `text.<设备>_area_N_polygon`。占用状态来自固件的 `binary_sensor.<设备>_area_N_occupied`。
- **`room_w` / `room_d`**: 房间的物理宽度和深度（单位：cm），用于在卡片绘图时缩放比例，不影响校准数据本身。您可以根据实际房间大小自行修改（例如 `300` 或 `500`）。

## 同帧坐标与轨迹连续性

将 `frame_entity` 配置为 ESPHome 的 Target Frame 文本传感器（参见 `tests/ld2450.yaml`），以读取同一 UART 数据帧中的 X/Y 坐标。配置后，空帧或无效帧会清空目标，不会回退到独立更新的坐标传感器；未配置时仍兼容旧版独立传感器。

新版组件通过可选的 `s` 数组保留 UART 原始槽位，避免一个目标消失后其他目标被重新编号。槽位并非永久人员身份：目标消失、更新间隔超过一秒，或位移超过 100 厘米加 300 厘米/秒乘以间隔时，会重新开始轨迹。该规则只控制绘图，不过滤或修改测量坐标，包括远处反射。
