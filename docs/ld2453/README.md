# LD2453 Lovelace Configuration Guide

[中文文档 (Chinese)](./README_CN.md)

This guide explains how to use the configuration file located in `tests/ld2453.yaml` within Home Assistant.

## 1. Introduction

`ld2453.yaml` contains the basic Lovelace card configuration for the Hi-Link LD2453 24 GHz 2D multi-target tracking radar using [mmwave-card](https://github.com/zomco/mmwave-card). It enables visual real-time tracking of up to 3 targets and geometric boundary calibration display for this radar in Home Assistant.
...
2. Your LD2453 radar device is successfully integrated into Home Assistant via ESPHome or other methods.
...
5. Copy the contents from `tests/ld2453.yaml` and paste them into the code editor.
...
cards:
  - type: custom:mmwave-card
    radar_model: ld2453
    presence_entity: binary_sensor.ld2453_presence
    target_1_x_entity: sensor.ld2453_target_1_x
    target_1_y_entity: sensor.ld2453_target_1_y
    target_1_speed_entity: sensor.ld2453_target_1_speed
    target_2_x_entity: sensor.ld2453_target_2_x
    target_2_y_entity: sensor.ld2453_target_2_y
    target_2_speed_entity: sensor.ld2453_target_2_speed
    target_3_x_entity: sensor.ld2453_target_3_x
    target_3_y_entity: sensor.ld2453_target_3_y
    target_3_speed_entity: sensor.ld2453_target_3_speed
    room_w: 400
    room_d: 350
```
...
All `entity_id`s in the configuration (such as `binary_sensor.ld2453_presence`) are examples. If your device has different names in Home Assistant, please make sure to replace them with **your actual entity IDs**.
...
- **`radar_model`**: `ld2453` (Fixed value, specifies the radar model used)
- **`presence_entity`**: [Required] Binary sensor indicating whether any target is present.
- **`target_n_x_entity`**: [Required] Radar-measured local X-axis coordinate for target n (Unit: cm). Please map for targets 1, 2, and 3.
- **`target_n_y_entity`**: [Required] Radar-measured local Y-axis coordinate for target n (Unit: cm). Please map for targets 1, 2, and 3.
