...
二维/三维型号可在校准的「实时」页画出最多三个占位区域。同步会写入 `text.<前缀>_area_1_polygon` … `_area_3_polygon`。质心近于 2 m 或短边小于 1.5 m 时卡片会警告。占用状态来自设备上的 `binary_sensor.<前缀>_area_N_occupied`，卡片自己不算。
